...
hardened-runtime/provisioning, Touch ID, and cold/warm Secure Enclave cache
checks remain a separate physical-machine release gate.
...
7. one touch permits a cold cache read, followed by a warm read with no second
   biometric prompt;
8. the signed/notarized package installs the bridge at
