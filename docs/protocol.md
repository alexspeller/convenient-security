...
`mode` is `built_in_memory`, `onboarding_import`, or
`external_temporary_file`. External mode must also include the canonical
absolute `externalEditorPath`; it is modeled as an unverified
named-plaintext-file consumer. Built-in and onboarding-import modes reject that
