...
public enum HostAuditService {
    ...
    public static func runReport(scanFilesystem: Bool, generatedAtHint: String? = nil) async -> HostAuditReport {
        let ctx = productionContext(scanFilesystem: scanFilesystem)
        return await HostAuditEngine().run(ctx, generatedAtHint: generatedAtHint)
    }
    ...
    /// it, so a later background run only flags a *change* from this point.
    public static func runInteractive(scanFilesystem: Bool, generatedAtHint: String? = nil) async -> HostAuditReport {
        let report = await runReport(scanFilesystem: scanFilesystem, generatedAtHint: generatedAtHint)
